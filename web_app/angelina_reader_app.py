...
def results(template):
    ...
    if ext in IMAGES or ext == "pdf":
        results_list = recognizer.run_and_save(
            request.values["img_path"],
            RESULTS_ROOT,
            target_stem=None,
            lang=request.values["lang"],
        ...
        results_list = recognizer.process_archive_and_save(
            request.values["img_path"],
            RESULTS_ROOT,
            lang=request.values["lang"],
            extra_info=extra_info,
...
def email(template):
    ...
    if form.validate_on_submit():
        ...
        title = (
            form.title.data or "Распознанный Брайль: " + Path(results_list[0][0]).with_suffix("").with_suffix("").name
        )
...
def run():
    ...
    parser.add_argument("--debug", dest="debug", action="store_true", help="enable debug mode (default: off)")
