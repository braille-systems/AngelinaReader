...
def create_model_retinanet(params, device):
    ...
    def detection_collate(batch):
        ...
        boxes = [
            ...
            * torch.tensor(params.data.net_hw[::-1] * 2, dtype=torch.float32, device=device)
            for b in batch
        ]
        ...
        if params.data.get_points:
            ...
            labels = [
                torch.tensor(
                    [[int(ch) - 1 for ch in label_tools.int_to_label010(int_lbl.item())] for int_lbl in lb],
                    dtype=torch.long,
                    device=device,
                )
                for lb in labels
        ...
        calc_cls_mask = torch.tensor([b[2].get("calc_cls", True) for b in batch], dtype=torch.bool, device=device)
        is_front_side_mask = torch.tensor([b[2]["is_front_side"] for b in batch], dtype=torch.bool, device=device)
        ...
        if original_images:  # inference mode
            return (
                inputs,
                (torch.stack(loc_targets), torch.stack(cls_targets), calc_cls_mask, is_front_side_mask),
                original_images,
            )
        else:
                ...
                for pred_i, handle_front_side in zip(pred, (True, False)):
                    ...
                    if this_side_mask.max() or calc_cls_mask_i.max():
                        ...
                        loss += retina_loss(
                            ...
                            cls_preds,
                            cls_targets,
                            loc_calc_mask=this_side_mask,
                            cls_calc_mask=calc_cls_mask_i,
                        )
            return loss
