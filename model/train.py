...
train_loader = data.create_dataloader(
    ...
    list_file_names=params.data.train_list_file_names,
    shuffle=True,
)
val_loaders = {
    k: data.create_dataloader(params, device=settings.device, collate_fn=collate_fn, list_file_names=v, shuffle=False)
...
metrics = OrderedDict(
    {
        "loss": ignite.metrics.Loss(
            loss.metric("loss"), batch_size=lambda y: params.data.batch_size
        ),  # loss calc already called when train
...
eval_metrics = OrderedDict(
    {
        "loss": ignite.metrics.Loss(
            loss, batch_size=lambda y: params.data.batch_size
        ),  # loss calc must be called when eval
        "loc": ignite.metrics.Loss(loss.metric("loc"), batch_size=lambda y: params.data.batch_size),
        "cls": ignite.metrics.Loss(loss.metric("cls"), batch_size=lambda y: params.data.batch_size),
    }
)
...
trainer = ovotools.ignite_tools.create_supervised_trainer(
    model, ctx.optimizer, loss, metrics=trainer_metrics, device=settings.device
)
...
if settings.findLR:
    ...
    def upd_lr(engine):
        log_lr = (
            params.lr_finder.log_lr_start
            + (params.lr_finder.log_lr_end - params.lr_finder.log_lr_start)
            * (engine.state.iteration - 1)
            / params.lr_finder.iters_num
        )
    ...
    def eval_accuracy(engine):
        if engine.state.epoch % 100 == 1:
            ...
            for key, data_list in data_set.items():
                acc_res = validate_retinanet.evaluate_accuracy(
                    os.path.join(ctx.params.get_base_filename(), "param.txt"), model, settings.device, data_list
                )
    ...
    if params.lr_scheduler.type == "clr":
        clr_scheduler = ovotools.ignite_tools.ClrScheduler(
            train_loader, model, ctx.optimizer, target_metric, params, engine=trainer
        )
    else:
...
if settings.findLR:
    ...
    best_model_buffer = ovotools.ignite_tools.BestModelBuffer(
        ctx.net, ctx.params.data.target_metric, minimize=False, params=ctx.params
    )
log_training_results = ovotools.ignite_tools.LogTrainingResults(
    evaluator=evaluator,
...
timer = ovotools.ignite_tools.IgniteTimes(
    trainer,
    count_iters=False,
    measured_events={
