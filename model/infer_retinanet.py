...
class BrailleInferenceImpl(torch.nn.Module):
    ...
    def forward(self, input_tensor, input_tensor_rotated, find_orientation, process_2_sides):
        ...
        loc_preds: List[torch.Tensor] = [torch.tensor(0)] * 8
        cls_preds: List[torch.Tensor] = [torch.tensor(0)] * 8
        if self.verbose >= 2:
            print("        forward.prepare", time.perf_counter() - t)
            t = time.perf_counter()
        ...
        if self.verbose >= 2:
            ...
            print("        forward.model", time.perf_counter() - t)
            t = time.perf_counter()
        if find_orientation:
            best_idx, err_score = self.calc_letter_statistics(cls_preds, self.cls_thresh, orientation_attempts)
            if self.verbose >= 2:
                print("        forward.calc_letter_statistics", time.perf_counter() - t)
                t = time.perf_counter()
        else:
        ...
        if process_2_sides:
            boxes2, labels2, scores2 = self.encoder.decode(
                loc_preds[best_idx + 2][0].cpu().data,
                cls_preds[best_idx + 2][0].cpu().data,
                (w, h),
                cls_thresh=self.cls_thresh,
                nms_thresh=self.nms_thresh,
                num_classes=self.num_classes,
        ...
        if self.verbose >= 2:
            ...
            print("        forward.decode", time.perf_counter() - t)
            t = time.perf_counter()
...
class BrailleInference:
    ...
    def run(
        ...
        if not isinstance(img, PIL.Image.Image):
            try:
                ...
                return None
        if self.verbose >= 2:
            print("run.reading image", time.perf_counter() - t)
        ...
        if self.verbose >= 2:
            ...
            print("run.run_impl", time.perf_counter() - t)
        return results_dict
    ...
    def run_impl(self, img, lang, draw_refined, find_orientation, process_2_sides, align, draw, gt_rects=[]):
        ...
        if self.verbose >= 2:
            print("    run_impl.make_batch", time.perf_counter() - t)
            t = time.perf_counter()
        ...
        if self.verbose >= 2:
            ...
            print("    run_impl.impl", time.perf_counter() - t)
            t = time.perf_counter()
        ...
        lines = postprocess.boxes_to_lines(
            ...
            scores=scores,
            lang=lang,
            filter_lonely=SAVE_FOR_PSEUDOLABELS_MODE != 1,
        ...
        if process_2_sides:
            ...
            lines2 = postprocess.boxes_to_lines(
                boxes2,
                labels2,
                scores=scores2,
                lang=lang,
        ...
        if self.verbose >= 2:
            print("    run_impl.postprocess", time.perf_counter() - t)
        ...
        if align and not process_2_sides:
            ...
            if self.verbose >= 2:
                print("    run_impl.align", time.perf_counter() - t)
        ...
        if draw:
            ...
            if self.verbose >= 2:
                print("    run_impl.draw", time.perf_counter() - t)

        if SAVE_FOR_PSEUDOLABELS_MODE == 1:
            # check that results are the same as raw rects
            shapes = results_dict["dict"]["shapes"]
            saved_boxes = [sh["points"] for sh in shapes]
            saved_labels = [lt.human_label_to_int(sh["label"]) for sh in shapes]
    ...
    def run_and_save(
        ...
        if result_dict is None:
            return None
        if self.verbose >= 2:
            print("run_and_save.run", time.perf_counter() - t)
            t = time.perf_counter()
        ...
        if self.verbose >= 2:
            print("run_and_save.save results", time.perf_counter() - t)
        return results
    ...
    def process_dir_and_save(
        ...
        for img_file, img_folder in zip(img_files, img_folders):
            ...
            ith_result = self.run_and_save(
                img_file,
                os.path.join(results_dir, img_folder),
                target_stem=None,
                lang=lang,
            ...
            for entry in archive.infolist():
                with archive.open(entry) as file:
                    ...
                    ith_result = self.run_and_save(
                        img,
                        results_dir,
                        target_stem=arch_name + "." + Path(entry.filename).stem,
                        lang=lang,
                        extra_info=extra_info,
                        draw_refined=draw_refined,
                        remove_labeled_from_filename=remove_labeled_from_filename,
...
if __name__ == "__main__":
    ...
    if SAVE_FOR_PSEUDOLABELS_MODE:
        ...
        results_dir = r"/home/orwell/Data/Braille/AngelinaDataset/pseudo/step_{}_opt_{}/{}".format(
            PSEUDOLABELS_STEP, SAVE_FOR_PSEUDOLABELS_MODE, folder
        )
