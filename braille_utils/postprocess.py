    ...
    def _calc_approximation(self, calc_chars, min_align_score):
        ...
        for i in range(len(calc_chars) - self.AVG_APPROX_DIST):
            ...
            for j in range(i + self.AVG_APPROX_DIST, len(calc_chars)):
                ...
                b = (calc_chars[j].x * calc_chars[i].y - calc_chars[i].x * calc_chars[j].y) / (
                    calc_chars[j].x - calc_chars[i].x
                )
    ...
    def refine(self):
        for i in range(len(self.chars)):
            ...
            if i > 0:
                ...
                curr_char.spaces_before = max(
                    0,
                    round(
                        0.5
                        * (
                            (curr_char.refined_box[0] + curr_char.refined_box[2])
                            - (prev_char.refined_box[0] + prev_char.refined_box[2])
                        )
                        / step
                    )
                    - 1,
...
def interpret_line_RU(line, lang, mode=None):
    ...
    for i, ch in enumerate(line.chars):
        ...
        if ch.char == letters.markout_sign:
            ...
            if digit_mode:
                ch.spaces_before += (
                    1  # need to separate from previous number or separate fraction part from integer part
                )
            digit_mode = True
            math_mode = True
                    ...
                    if ch.char is None:
                        ...
                        if ch.char is not None:
                            if ch.char == "/0" and prev_ch.labeling_char == "0":
                                ...
                                ch.labeling_char = (
                                    ch.char
                                ) = None  # frac_mode = True # TOOD вернуться к отображению дробей
                else:
                    ch.labeling_char = ch.char = lt.int_to_letter(ch.label, ["NUM_DENOMINATOR"])
                    if ch.char is not None:
                        ch.char = ch.char[1:]
            ...
            if ch.char is None:
                if (
                    ...
                    or not prev_ch
                    or prev_ch
                    and not prev_ch.char.isalpha()
                    or math_mode
                    and not math_lang
                    and lt.int_to_letter(ch.label, ["MATH_RU"]) == ".."
                ):
                ...
                if not math_lang and (ch.spaces_before or lt.int_to_letter(ch.label, ["MATH_RU"]) == ".."):
                    ...
                    if ch.char is not None:
                        if ch.char == "..":
                            if (
                                i < len(line.chars) - 1
                                and line.chars[i + 1].spaces_before == 0
                                and lt.int_to_letter(line.chars[i + 1].label, ["NUM"]) is not None
                            ):
                                ch.char = "."
                            else:
                                ch.char = "*"
            ...
            if ch.char == "()":
                ...
                ch.char == "ь"
                and i < len(line.chars) - 1
                and (line.chars[i + 1].spaces_before or True)  # TODO
                and brackets_on["["] > 0
            ):
                ch.char = "]"
                brackets_on["["] -= 1
...
def filter_lonely_rects_for_lines(lines):
    ...
    filtered_chars = []
    for ln in lines:
        while len(ln.chars) and (
            ln.chars[0].label not in allowed_lonely
            and (len(ln.chars) > 1 and ln.chars[1].spaces_before > 1 or len(ln.chars) == 1)
        ):
        ...
        while len(ln.chars) and (
            ln.chars[-1].label not in allowed_lonely
            and (len(ln.chars) > 1 and ln.chars[-1].spaces_before > 1 or len(ln.chars) == 1)
        ):
...
def find_transformation_full(lines):
    ...
    for ln in lines:
        for ch in ln.chars:
            bounds = [
                min(bounds[0], ch.refined_box[0]),
                min(bounds[1], ch.refined_box[1]),
                max(bounds[2], ch.refined_box[2]),
                max(bounds[3], ch.refined_box[3]),
            ]
    ...
    print(best_errors)
    if (
        best_errors[0] is not None
        and best_errors[1] is not None
        and best_errors[2] is not None
        and best_errors[3] is not None
    ):
        src_points = np.array(
            [
                find_cross(best_lines[0], best_lines[1]),
                find_cross(best_lines[2], best_lines[1]),  # 0 1
                find_cross(best_lines[0], best_lines[3]),
                find_cross(best_lines[2], best_lines[3]),
            ]
        )  # 2 3
        src_bounds = [
            min(src_points[0, 0], src_points[2, 0]),
            min(src_points[0, 1], src_points[1, 1]),
            max(src_points[1, 0], src_points[3, 0]),
            max(src_points[2, 1], src_points[3, 1]),
        ]  # left top r. bot.
        ...
        hom = cv2.getRotationMatrix2D(((bounds[0] + bounds[2]) / 2, (bounds[1] + bounds[3]) / 2), angle, 1.0)
    else:
        hom = None
    return hom
    ...
    if len(lines) > 0:
        ...
        if sum_weights > 0:
            ...
            if abs(angle) < MAX_ROTATION and abs(angle) > MIN_ROTATION:
                ...
                scale = min(
                    center[0] / (center[0] - new_points[0][0][0]),
                    center[1] / (center[1] - new_points[0][0][1]),
                    (img_size_wh[0] - center[0]) / (new_points[0][1][0] - center[0]),
                    center[1] / (center[1] - new_points[0][1][1]),
                    center[0] / (center[0] - new_points[0][2][0]),
                    (img_size_wh[1] - center[1]) / (new_points[0][2][1] - center[1]),
                    (img_size_wh[0] - center[0]) / (new_points[0][3][0] - center[0]),
                    (img_size_wh[1] - center[1]) / (new_points[0][3][1] - center[1]),
                )
                if scale < 1:
                    hom = cv2.getRotationMatrix2D(center, angle * 59, scale)
    return hom
