...
def common_aug(mode: str, params: AttrDict):
    ...
    if mode == AugMode.train.value:
        ...
        augs_list.append(
            albumentations.RandomCrop(height=params.data.net_hw[0], width=params.data.net_hw[1], always_apply=True)
        )
    ...
    return albumentations.ReplayCompose(
        augs_list, p=1.0, bbox_params={"format": "albumentations", "min_visibility": 0.5}
    )
...
class ImagePreprocessor:
    ...
    def random_resize_and_stretch(self, img, new_width_range, stretch_limit=0):
        ...
        img_max_sz = img.shape[
            1
        ]  # max(img.shape[0]*stretch, img.shape[1]) #img.shape[1]  # GVNC - now it is resizing to max
    ...
    def to_normalized_tensor(self, img, device):
        ...
        std = torch.max(
            ten_img.view(3, -1).std(dim=1), torch.tensor(self.params.data.get("max_std", 0) * 255).to(ten_img)
        )
    ...
    def __init__(self, params, device, list_file_names, mode, verbose):
        ...
        for list_file_name in list_file_names:
            ...
            if sample_weight > 1e-10:
                sub_datasets.append(
                    BrailleSubDataset(params, device, list_file_name, mode, verbose, sample_weight, list_params)
                )
...
class BrailleSubDataset:
    ...
    def filter_data_by_scores(self, scores_filter, list_file_name):
        ...
        if len_before != len(self.image_files):
            print(
                "filtering " + list_file_name + " for pseudolabeling: ",
                scores_filter,
                len_before,
                "->",
    ...
    def __getitem__(self, item):
        ...
        if self.mode == "train":
            return (
                self.image_preprocessor.to_normalized_tensor(aug_img, device=self.device),
                np.asarray(aug_bboxes).reshape(-1, 6),
                sample_params,
            )
        else:
            return (
    ...
    def read_annotation(self, label_filename, width, height):
        ...
        if ext == "txt":
            return dsbi.read_DSBI_annotation(
                label_filename,
                width,
                height,
...
def create_dataloader(params, device, collate_fn, list_file_names, shuffle, mode="train", verbose=0):
    ...
    loader = torch.utils.data.DataLoader(
        dataset, params.data.batch_size, shuffle=shuffle, num_workers=0, collate_fn=collate_fn
    )
    return loader
...
if __name__ == "__main__":
    ...
    assert rect_hflip((0, 1, 2, 3, lt.label010_to_int("111000"),)) == (
        0,
        1,
        2,
        3,
        lt.label010_to_int("000111"),
    )
    assert rect_hflip((0, 1, 2, 3, lt.label010_to_int("000011"),)) == (
        0,
        1,
        2,
        3,
        lt.label010_to_int("011000"),
    )
    assert rect_hflip((0, 1, 2, 3, lt.label010_to_int("001100"),)) == (
        0,
        1,
        2,
        3,
        lt.label010_to_int("100001"),
    )

    assert rect_vflip((0, 1, 2, 3, lt.label010_to_int("111100"),)) == (
        0,
        1,
        2,
        3,
        lt.label010_to_int("111001"),
    )
    assert rect_vflip((0, 1, 2, 3, lt.label010_to_int("001011"),)) == (
        0,
        1,
        2,
        3,
        lt.label010_to_int("100110"),
    )

    params = AttrDict(data=AttrDict(batch_size=2, get_points=False, rect_margin=0.3))
